...
void Land::update(float dt)
{
	...
	{
		m_grounds[0].setPosition(m_grounds[1].getPosition().x + m_grounds[1].getGlobalBounds().width, m_grounds[0].getPosition().y);
	}
	else if (m_grounds[1].getPosition().x + m_grounds[1].getGlobalBounds().width < 0)
	{
		m_grounds[1].setPosition(m_grounds[0].getPosition().x + m_grounds[0].getGlobalBounds().width, m_grounds[1].getPosition().y);
	}
}
